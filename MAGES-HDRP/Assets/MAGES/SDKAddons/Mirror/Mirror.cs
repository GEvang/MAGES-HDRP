...
public class Mirror : MonoBehaviour
{
    ...
    public Vector3 projectionDirection = Vector3.up;
    ...
    void UpdateCamera(ScriptableRenderContext SRC, Camera camera)
    {
            ...
            if (UniversalRenderPipeline.IsStereoEnabled(camera))
            {
                try
                {
    ...
    private void RenderCamera(Camera camera, Renderer rend, Camera.StereoscopicEye eye, ref RenderTexture portalTexture, ScriptableRenderContext SRC)
    {
        ...
        Vector3 normal = transform.TransformDirection(projectionDirection); 
        ...
        bool isStereoEyeValid = (eye == Camera.StereoscopicEye.Left) ? leftEyeState.tracked:rightEyeState.tracked;

        if (UniversalRenderPipeline.IsStereoEnabled(camera) && isStereoEyeValid)
        {
            Vector3 eyeOffset;
    ...
    private void CreatePortalCamera(Camera currentCamera, Camera.StereoscopicEye eye, out Camera portalCamera, ref RenderTexture portalTexture)
    {
        ...
        if (!portalTexture || m_OldReflectionTextureSize != m_TextureSize) // if it doesn't exist or the size has changed
        {
            ...
            portalTexture.hideFlags = HideFlags.DontSave;

            portalTexture.antiAliasing = 2;

            m_OldReflectionTextureSize = m_TextureSize; // save the old texture size
        }

        // Create camera with the render texture
        if (!m_PortalCameras.TryGetValue(currentCamera, out portalCamera)) // if it does not yet exist in the dictionary, create it. If it does, assign it.
        {
