Shader "Unlit/Visualization_GridPlane"
{
    Properties
    {
        ...
        _Thickness("Thickness", Range(0.0001, 0.1)) = 0.005
        _SecondaryFadeInSpeed("Secondary Fade In Speed", Range(0.01,0.1)) = 0.5
        _OffsetX("Offset X", Float) = 0.0
        _OffsetY("Offset Y", Float) = 0.0
        _RadialFadeAmount("Radial Fade Amount", Range(0.0,10.0)) = 0.0
        _TotalFade("Total Fade", Range(0.0,1.0)) = 1.0
    }
    SubShader
    {
        ...
        Pass
        {
            ...
            v2f vert (appdata v)
            {
                ...
                UNITY_INITIALIZE_VERTEX_OUTPUT_STEREO(v.vertex);
            ...
            fixed4 frag (v2f i) : SV_Target
            {
            	...
            	// With the ceil value of the log10 of the scale,f we obtain the closest measure unit above (eg : 165 -> 3, 0.146 -> 0, 0.001 -> -3)
            	...
            	float fade =  pow(1 - maprange(logMappedScale, 0.1, 1, 0.00001, 0.99999), _SecondaryFadeInSpeed);
            	...
            	pos.x = floor(frac((uv.x - 0.5 * _Thickness) * localScale) + _Thickness * localScale);
            	pos.y = floor(frac((uv.y - 0.5 * _Thickness) * localScale) + _Thickness * localScale);

            	if (pos.x == 1 || pos.y == 1) {
            	   col.a = max((1 - fade), fade);
            	} else {
            	   pos.x = floor(frac((uv.x - 0.5 * _Thickness) * 10.0 * localScale) + _Thickness * 10.0 * localScale);
            	   pos.y = floor(frac((uv.y - 0.5 * _Thickness) * 10.0 * localScale) + _Thickness * 10.0 * localScale);
            	...
            	float2 npos = (2 * (i.uv + 0.5)) - 1;

            	float ctest = (pow(abs(length(i.uv) - 1)*2, _RadialFadeAmount*_RadialFadeAmount));
