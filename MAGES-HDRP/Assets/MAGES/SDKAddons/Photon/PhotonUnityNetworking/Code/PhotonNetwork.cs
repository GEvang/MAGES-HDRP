...
namespace Photon.Pun
{
    ...
    public struct InstantiateParameters
    {
        ...
        public bool setCustomPositionAndRotation;
        ...
        {
            this.setCustomPositionAndRotation = true;
        ...
        {
            this.setCustomPositionAndRotation = false;
        ...
        public InstantiateParameters(string prefabName,GameObject parent, byte @group, object[] data, byte objLevelPrefix, int[] viewIDs, Player creator, int timestamp)
        {
            this.setCustomPositionAndRotation = false;
            this.position = Vector3.zero;
            this.rotation = Quaternion.identity;
    ...
    public static partial class PhotonNetwork
    {
        ...
        public static GameObject Instantiate(string prefabName, byte group = 0, object[] data = null)
        {
            ...
            Pun.InstantiateParameters netParams = new InstantiateParameters(prefabName, group, data, currentLevelPrefix, null, LocalPlayer, ServerTimestamp);
            return NetworkInstantiate(netParams, false);
        }
        
        public static GameObject Instantiate(string prefabName,GameObject parent, byte group = 0, object[] data = null)
        {
            if (CurrentRoom == null)
            {
                Debug.LogError("Can not Instantiate before the client joined/created a room. State: "+PhotonNetwork.NetworkClientState);
            ...
            Pun.InstantiateParameters netParams = new InstantiateParameters(prefabName,parent, group, data, currentLevelPrefix, null, LocalPlayer, ServerTimestamp);
            return NetworkInstantiate(netParams, false);
        }
        ...
        private static GameObject NetworkInstantiate(Pun.InstantiateParameters parameters, bool sceneObject = false, bool instantiateEvent = false)
        {
            ...
            if(parameters.setCustomPositionAndRotation)
                go = prefabPool.Instantiate(parameters.prefabName, parameters.position, parameters.rotation);
            else if(!parameters.parent){
                go = prefabPool.Instantiate(parameters.prefabName);
            }
            else
            {
                go = prefabPool.Instantiate(parameters.prefabName,parameters.parent);
            }
            parameters.position = go.transform.position;
            parameters.rotation = go.transform.rotation;
        ...
        public static string FindPunAssetFolder()
        {
            ...
            return "Assets/MAGES/ExternalAssets/SDKAddons/Photon/PhotonUnityNetworking/";
        }
