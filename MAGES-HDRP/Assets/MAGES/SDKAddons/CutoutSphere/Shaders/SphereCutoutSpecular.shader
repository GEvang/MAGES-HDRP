Shader "Custom/SphereCutoutSpecular"
{
	...
	Properties
	{
		...
		[NoScaleOffset]_Specular("Specular", 2D) = "white" {}		// Specular
		_Shininess("Shininess", Range(0 , 2)) = 0				// Smoothness
		...
		{  // Rendered in sync with geometry shader as opaque
			...
			#pragma target 3.0
			#pragma surface mySurf StandardSpecular  addshadow fullforwardshadows
			struct Input
			{
				float2 uv_texcoord;
				float3 worldPos;
			...
			void mySurf(Input i, inout SurfaceOutputStandardSpecular o)
			{
				...
				o.Normal = UnpackNormal(tex2D(_Normal, uvCoordinates));												 // Unpack compressed normal map
				o.Albedo = (_Albedo_tint * tex2D(_Albedo, uvCoordinates)).rgb;										 // Apply albedo with its tint
				...
				o.Specular =  (tex2D(_Specular, uvCoordinates)).rgb  * _Shininess  ;							     // Apply specular with its multiplier and use the r channel (standard shader uses this for metallic)
				o.Smoothness = _Smoothness;																				 // Apply smoothness   <('_'<)  "please don't mind the dancing figure" ¯\_(ツ)_/¯
				o.Alpha = 1.0;																				 // Apply 1 to alpha   (>'_')> <('_')>   
				...
				clip(cutoutMask - _Cutoff);																				 // Discard pixels if cutoutMask-Cutoff < 0
			}
			ENDCG
		}
			Fallback "Diffuse"
